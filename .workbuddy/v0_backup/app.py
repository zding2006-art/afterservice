...
def upload_file():
    ...
    try:
        df, results, summary, is_multi, month_details = run_all_analysis(filepath)
        ...
        if is_multi and month_details:
            ...
            if already:
                ...
                save_monthly(summary["year"], summary["month"], summary["month_label"], summary, results)
                return jsonify({
                    "success": True,
                    "is_multi_month": False,
    ...
    try:
        ...
        for m in existing:
            if m["year"] == year and m["month"] == month:
                delete_month(m["id"])
                break
        save_monthly(year, month, month_label, summary, results)
...
def export_excel(month_id):
    ...
    section_map = {
        ...
        "analysis_12": ("十三、座盖故障使用月数分布", ["使用月数区间", "发生次数", "累计次数"]),
    }
...
def ai_insights():
    ...
    if not data.get("details") or len(data["details"]) < 2:
        return jsonify({
            "summary": "数据不足（需要至少 2 个月的数据），无法进行 AI 分析。",
            "key_findings": [],
            "risk_alerts": [],
            "recommendations": [],
            "analysis_angles": [],
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5859, debug=not IS_FROZEN)
