...
# ── CORS 配置 ──────────────────────────────────────────────
# 通过 Nginx 反向代理时，前端和后端同源，不需要 CORS。
# 直接用 IP 访问时（如 http://1.2.3.4:5859），前端页面由 Flask 提供，也是同源，不需要 CORS。
# 只有开发时（file:// 或 localhost 与后端不同端口）才需要 CORS。
# CORS_ORIGINS 环境变量：逗号分隔的允许来源列表
# 设为 "*" 可允许所有来源（不推荐生产环境，但可用于排查问题）
