...
def save_monthly(year, month, month_label, summary, results):
    """保存当月分析结果"""
    conn = get_db()
    conn.execute("""
    INSERT OR REPLACE INTO monthly_summary
        (year, month, month_label, total_records, total_cost,
         in_warranty_count, out_warranty_count, cover_count, whole_count, analysis_data)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
def _build_trends(months_data):
    ...
    for m in months_data:
        ...
        for row in (ad or {}).get("analysis_12", {}).get("data", []) if ad else []:
            ...
            if bucket and bucket not in usage_buckets_order:
                usage_buckets_order.append(bucket)
    seat_usage_trend = {
